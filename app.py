...
def copy_card_component(num: str, count: int):
    badge = "<span style='margin-left:8px;padding:2px 8px;border-radius:999px;background:#e5e7eb;font-size:12px;'>aparece " + str(count) + "×</span>" if count>1 else ""
    html = (
...
# Botão para atualizar/forçar reload
...
with c3:
    # Date input para Data (de Início)
    min_d = df["Data"].min()
    max_d = df["Data"].max()
    default_d = min_d if pd.notna(min_d) else date.today()
    data_sel = st.date_input("Data (de Início)", value=default_d, min_value=min_d if pd.notna(min_d) else None, max_value=max_d if pd.notna(max_d) else None)
